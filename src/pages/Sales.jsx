...
/* ─── KPI Drawer ────────────────────────────────────────────────── */
/** Breakdown data shown when a KPI card is clicked. */
const KPI_DETAILS = {
...
/** Static sales funnel stage data (replace with API when available). */
const FUNNEL = [
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'4px'}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                        ...
                        {q._apiStatus==='draft' && q.id && (
                          <button style={{fontSize:'12.5px',fontWeight:600,color:'#2563eb',background:'none',border:'none',cursor:isActionPending(`${q.id}:send`)?'not-allowed':'pointer',padding:0,opacity:isActionPending(`${q.id}:send`)?0.5:1}}
                            disabled={isActionPending(`${q.id}:send`)}
                            onClick={e=>{e.stopPropagation();sendToClient(q);}}>
                            {isActionPending(`${q.id}:send`) ? 'Sending…' : 'Send →'}
                          </button>
                        )}
                        {q._apiStatus==='sent' && (
                          <>
                            <button style={{fontSize:'12.5px',fontWeight:600,color:'#16a34a',background:'none',border:'none',cursor:isActionPending(`${q.id}:accept`)?'not-allowed':'pointer',padding:0,opacity:isActionPending(`${q.id}:accept`)?0.5:1}}
                              disabled={isActionPending(`${q.id}:accept`)}
                              onClick={e=>{e.stopPropagation();acceptQuote(q);}}>Accept</button>
                            <button style={{fontSize:'12.5px',fontWeight:600,color:'#b91c1c',background:'none',border:'none',cursor:isActionPending(`${q.id}:reject`)?'not-allowed':'pointer',padding:0,opacity:isActionPending(`${q.id}:reject`)?0.5:1}}
                              disabled={isActionPending(`${q.id}:reject`)}
                              onClick={e=>{e.stopPropagation();rejectQuote(q);}}>Reject</button>
                          </>
                        )}
                        {q._apiStatus==='accepted' && (
                          <button style={{fontSize:'12.5px',fontWeight:600,color:'#7c3aed',background:'none',border:'none',cursor:isActionPending(`${q.id}:convert`)?'not-allowed':'pointer',padding:0,opacity:isActionPending(`${q.id}:convert`)?0.5:1}}
                            disabled={isActionPending(`${q.id}:convert`)}
                            onClick={e=>{e.stopPropagation();convertToOrder(q);}}>
                            {isActionPending(`${q.id}:convert`) ? 'Converting…' : 'Convert →'}
                          </button>
                        )}
                        {q.id && (
